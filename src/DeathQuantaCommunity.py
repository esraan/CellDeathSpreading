...
class DeathQuantaCommunity(DeathQuanta):
    def __init__(self,
                cells_xy: np.ndarray, 
    ...
    def create_scramble(self, level=1):
        self.permuted_death_time_communities = []
        for i in range(self.n_scramble):
            self.permuted_death_time_communities.append(self.get_TOD_from_nucleator(level, permute=True))
         
    def find_communities(self, level):
